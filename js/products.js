...
export async function getAllProducts() {
  const products1 = await get("products", "products");
  const products2 = await get("products", "products2");
...
export async function getProductById(id) {
  ...
  return await get("products", `${endpoint}/${id}`);
}
...
export async function createProduct(productData) {
  return await post("products", "products2", productData);
}

// Atualizar produto
export async function updateProduct(id, data) {
  const endpoint = getEndpointById(id);
  return await put("products", `${endpoint}/${id}`, data);
}

// Deletar produto
export async function deleteProduct(id) {
  const endpoint = getEndpointById(id);
  return await del("products", `${endpoint}/${id}`);
}